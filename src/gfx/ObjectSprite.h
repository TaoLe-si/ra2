...
/// 朝向档数。原版 256 分度、HVA 每模型 32 帧。
/// 【半 RE】"烘焙档数 = HVA 帧数"是推断——原版逐朝向动态渲染（无烘焙），
/// 32 档只是等价近似。逆向任务：TechnoClass::Draw 的朝向量化粒度。
constexpr int kFacingSteps = 32;

/// 一个体素占几个像素。【半 RE】48 voxels/格 是 TS/RA2 模型惯例
///（社区共识 + HVA 平移 ×12 的 det 关系自洽），不是从 gamemd 直接读出的
/// 常数；60/48 = 1.25 是换算值。旧值 8.0 无任何依据（曾致车辆大 6.4 倍）。
/// 逆向任务：0x753D00 投影调用链里体素坐标→屏幕像素的实际乘数
///（0x7586F0 入口参数）。
