...
//
// 【光向量朝向 —— 已按 exe 真值回正，不再翻转】
//   0x00754C00 把 (-0.7071041, -0.7071041, 0) 过一次 3×4 变换（矩阵由
//   0x5AE860 / 0x5AF080 用参数 0.785375 ≈ π/4 运行时算出）写进 0x00887470。
//   仰角来自 0x52BDD1：[0x88741c] = 0.9599 rad = 55°（asin(0.8165)=54.7°，
//   与 normalize(-1,-1,2) 的 z 分量自洽）。默认值 = normalize(-1,-1,2)。
//
//   【为什么曾经翻转、现在改回来】历史版本把水平方位绕 Z 转 180°
//   （normalize(1,1,2)），理由是"这个方位下镜头侧两个面背光，47% 像素压最暗级"。
//   代价是**阴影投影也跟着转了**：影子投向画面上方、被车体自身完全挡住，
//   表现为"游戏内单位没影子"。原版方位 (-1,-1) 的影子投向画面**右下**，
//   永远露在车体外 —— 这才是原版单位脚下影子的来源。镜头侧背光偏暗
//   正是原版观感（RA2 单位朝镜头的面就是暗的），按约定以 exe 真值为准。
...
struct VoxelLight {
    ...
    /// = (方位 -0.7071,-0.7071 抄自 0x754C00 初始化 + 仰角 55° 抄自 0x52BDD1)。
...
struct VoxelShadow {
    ...
    float alpha = 0.45f;     ///< **【估计】0..1，1 = 全黑；RE 待补**
};
