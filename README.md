...
| 反汇编 | 1,392,506 条指令 |
| 函数 | **19,533 个**（分档播种，9 条证据通道，每个入口都带 `tier`）；见 §3.3 |
...
| 函数边界 | **19,533 个函数**（`src/re/FuncTable.h`）；`0x401000–0x7C0000` 段入口 **100%** 落在 16 字节边界（18,945/18,945，两条独立通道各算一遍）；区间两两不重叠；`.text` 字节账 代码 93.3% / 填充 3.6% / 数据 0.2% / **未知 2.81%**（不猜不填，逐段列出） |
